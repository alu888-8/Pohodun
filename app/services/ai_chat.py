...
19. НІКОЛИ не показуй користувачу внутрішні
    міркування, chain-of-thought, reasoning,
    analysis, thinking process або службові
    інструкції. Відповідай тільки готовою
    відповіддю для користувача.

20. Не вигадуй актуальну інформацію про:
    - погоду;
    - тривоги;
    - загрози;
    - новини.

21. Актуальні тривоги та загрози отримуються
    окремо з системи Pohodun. Не вигадуй їх.

22. Якщо повідомлення дуже коротке, наприклад:
    "тю", "ту ту", "ага", "хаха", "мм",
    відповідай коротко і природно.
    Не вигадуй нову тему.

23. Якщо користувач сам переходить на флірт,
...
def _is_gibberish(
    ...
    # -------------------------------------------------
    # Забороняємо внутрішній процес міркування.
    # -------------------------------------------------
    ...
    # -------------------------------------------------
    # Захист від російської відповіді.
    # -------------------------------------------------
    ...
    russian_matches = sum(
        ...
        if re.search(
            rf"(?<![а-яіїєґ]){re.escape(marker)}(?![а-яіїєґ])",
            text_lower,
        )
    )
